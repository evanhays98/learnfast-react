...
def getToken():
    data = {
        "mail": "evan.hays@test.fr",
        "password": "12345"
    }
    res = requests.post("http://192.168.1.23:3003/auth/signin", data=data)
    return res.json()["access_token"]


def addSentenceToApi(content, trad, indication, chapterID, token):
    data = {
        "content": content,
        "traduction": trad,
    ...
    response = requests.post("http://192.168.1.23:3003/sentence/addSentence", data=data, headers=headers)


text = ""

def addSentence(file):
    f = open(file, "r")
    text = f.read()
    sentences = []
    res = ""
    ...
    for sentence in sentences:
        for i in range(len(sentence)):
            ...
            if (sentence[i] == '}'):
                ...
                print(res + " -> " + (str)(translator.translate(res, lang_src='en',lang_tgt='fr')))
                res = ""
                continue
            if (braceStart):
                res += sentence[i]
...
def test():
    ...
    title = soup.find('Result-Icon-Container')
    print(title)


if __name__ == '__main__':
    #tok = getToken()
    #addSentence('je {mange} des cookies', 'eat', 'verbe', 18, tok)
    #translate_text = translator.translate('hello', lang_src='en',lang_tgt='fr')
    #print(translate_text)
    #addSentence("SpidersAlongtheDark.txt")
    test()
